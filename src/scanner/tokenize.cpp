...
void read_float_or_int_literal(const std::string& code, std::size_t& i,
...
void read_string_literal(const std::string& code, std::size_t& i,
                         tkn::Point& current_position,
                         std::deque<tkn::TokenInfo>& tokens) {

  auto [name, new_i] =
      read_string<StringLiteralConverter>(code, i + 1, string_literal_filter);

  if (new_i + 1 >= code.size() || code[new_i + 1] != '"') {
    std::cerr << "Unterminated string literal at line " << current_position.line
...
void read_language_symbol(const std::string& code, std::size_t& i,
                          tkn::Point& current_position,
                          std::deque<tkn::TokenInfo>& tokens) {
  ...
  if (it == std::end(language_symbols_table)) {
    std::cerr << "Unexpected character: " << code[i] << " at line "
              << current_position.line << ", offset " << current_position.offset
              << "." << std::endl;
...
auto tokenize(const std::string& code) -> std::deque<tkn::TokenInfo> {
  ...
  tokens.emplace_back(
      tkn::EOFToken{},
      tkn::Position{current_position.line, current_position.offset + 1, 0});
