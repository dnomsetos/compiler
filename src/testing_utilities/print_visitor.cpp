...
void PrintVisitor::operator()(const ast::CastNode& cast_node) {
  if (skip_empty_) {
    if (!cast_node.type.has_value()) {
      operator()(*cast_node.expression);
      return;
    }
  }

  out_ << tabs_ << "cast: " << std::endl;

  tabs_ += "  ";
  operator()(*cast_node.expression);

  if (cast_node.type.has_value()) {
    out_ << tabs_ << "to type: " << std::endl;

    tabs_ += "  ";
    operator()(*cast_node.type.value());
